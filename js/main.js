...
class FontAnimationApp {
  constructor() {
    ...
    this.canvasBackground =
      window.AppSettings?.getValue("canvasBackground") || "#ffffff";
    this.zoom = 1;
    this.panX = 0;
    this.panY = 0;
    this.history = [];
  ...
  checkBrowserCompatibility() {
    ...
    if (!window.FontFace) {
      incompatibleFeatures.push(
        "FontFace API (required for custom font loading)",
      );
    }
  ...
  setupEventListeners() {
      ...
      if (newWidth > maxWidth) {
        ...
        if (window.UIManager) {
          window.UIManager.createNotification(
            `Canvas width limited to 4K maximum (${maxWidth}px)`,
            "warning",
          );
        }
      }
    ...
    document.getElementById("canvasHeight").addEventListener("change", (e) => {
      ...
      if (newHeight > maxHeight) {
        ...
        if (window.UIManager) {
          window.UIManager.createNotification(
            `Canvas height limited to 4K maximum (${maxHeight}px)`,
            "warning",
          );
        }
      }
    ...
    document
      .getElementById("canvasBackgroundColor")
    ...
    document
      ...
      .addEventListener("click", () => this.openProject());
    document
      .getElementById("saveBtn")
      .addEventListener("click", () => this.saveProject());
    document
      .getElementById("openFileInput")
      .addEventListener("change", (e) => this.loadProject(e));

    // Tools
    document
      .getElementById("handTool")
      .addEventListener("click", () => this.setTool("hand"));
    document
    ...
    document
      ...
      .addEventListener("click", () => this.showAboutModal());
  ...
  setupRightPanelEventListeners() {
        ...
        if (this.fonts.has(e.target.value)) {
          ...
          if (fontInfo.variableAxes) {
            ...
            Object.entries(fontInfo.variableAxes).forEach(([tag, axisInfo]) => {
              ...
              if (
                !this.selectedObject.initialState.hasOwnProperty(propertyName)
              ) {
                this.selectedObject.initialState[propertyName] =
                  axisInfo.default;
              }
            });
          }
    ...
    document.getElementById("lineHeight").addEventListener("input", (e) => {
      if (this.selectedObject) {
        const lineHeight = Math.max(0.1, parseFloat(e.target.value) || 1.2);
        this.updateObjectProperty(
          this.selectedObject,
          "lineHeight",
          lineHeight,
    ...
    document.getElementById("lineHeight").addEventListener("change", () => {
      this.saveState();
    });
    ...
    document.querySelectorAll(".transition-btn").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        if (this.selectedObject && window.TransitionEditor) {
          window.TransitionEditor.openModal(
            this.selectedObject,
            property,
            this,
          );
  ...
  setupTimelineRulerClick() {
    ...
    const getFrameFromEvent = (e) => {
      ...
      const pixelsPerFrame =
        this.timeline.calculateTimelineWidth() / this.totalFrames;
  ...
  duplicateObject(sourceObject) {
    ...
    Object.keys(sourceObject.keyframes).forEach((property) => {
      duplicate.keyframes[property] = sourceObject.keyframes[property].map(
        (keyframe) => ({
  ...
  handleHandToolDrag(event) {
    if (this.panStartMouseX === undefined || this.panStartMouseY === undefined)
      return;
  ...
  initializeUIFromSettings() {
    ...
    if (window.AppSettings) {
      ...
      document.getElementById("canvasBackgroundColor").value =
        this.canvasBackground;
  ...
  setupKeyboardShortcuts() {
    ...
    document.addEventListener("keydown", (e) => {
      ...
      if (e.key.toLowerCase() === "escape") {
        ...
        const exportManager = window.getExportManager
          ? window.getExportManager()
          : null;
  ...
  setTool(tool) {
    ...
    this.canvas.className = this.canvas.className.replace(
      /\b\w+-cursor\b/g,
      "",
    );
  ...
  getPropertyValue(textObject, property, frame = this.currentFrame) {
    ...
    if (
      ...
      textObject._tempValues.hasOwnProperty(property)
    ) {
    ...
    if (!keyframes || keyframes.length === 0) {
      ...
      if (
        ...
        textObject.initialState.hasOwnProperty(property)
      ) {
    ...
    if (beforeKeyframe && afterKeyframe) {
      let progress =
        (frame - beforeKeyframe.frame) /
        (afterKeyframe.frame - beforeKeyframe.frame);
      ...
      if (property === "color") {
        ...
        return (
          beforeKeyframe.value +
          (afterKeyframe.value - beforeKeyframe.value) * progress
        );
      }
    }
  ...
  measureTextWithFeatures(textObject, props) {
    ...
    try {
      ...
      const fontAvailable =
        this.fonts.has(textObject.fontFamily) &&
        this.isFontLoaded(textObject.fontFamily);

      if (fontAvailable) {
        // Apply variable font axes and/or OpenType features if available
        const hasVariableAxes =
          props.variableAxes && Object.keys(props.variableAxes).length > 0;
        const hasOpenTypeFeatures =
          props.openTypeFeatures &&
          Object.keys(props.openTypeFeatures).length > 0;

        if (hasVariableAxes || hasOpenTypeFeatures) {
          // Apply font features to the temporary canvas
          this.applyFontFeaturesToTempCanvas(
            tempCanvas,
      ...
      return this.measureMultilineTextWithContext(
        tempCtx,
        textObject.text,
        props,
      );
  ...
  getLineHeightPixels(props) {
    const lineHeightMultiplier =
      Number.isFinite(props.lineHeight) && props.lineHeight > 0
        ? props.lineHeight
        : 1.2;
    return props.fontSize * lineHeightMultiplier;
  }

  measureMultilineTextWithContext(context, text, props) {
    const lines = this.getTextLines(text);
    const lineHeightPx = this.getLineHeightPixels(props);
    let maxWidth = 0;
    ...
    return {
      width: maxWidth,
      height: props.fontSize + Math.max(0, lines.length - 1) * lineHeightPx,
      lineCount: lines.length,
      lineHeightPx,
    };
  }

  drawMultilineText(text, x, y, lineHeightPx) {
    const lines = this.getTextLines(text);
    lines.forEach((line, index) => {
      this.ctx.fillText(line, x, y + index * lineHeightPx);
    });
  }

  isSafariBrowser() {
    const ua = navigator.userAgent;
    return /^((?!chrome|chromium|android|crios|fxios|edgios).)*safari/i.test(
      ua,
    );
  }

  hasEnabledStylisticSet(openTypeFeatures = {}) {
    return Object.entries(openTypeFeatures).some(
      ([tag, enabled]) => enabled && /^ss\d{2}$/.test(tag),
    );
  }

  getEnabledFeatureFlags(openTypeFeatures = {}) {
    return Object.entries(openTypeFeatures).reduce((features, [tag, enabled]) => {
      if (enabled) {
        features[tag] = true;
      }
      return features;
  ...
  shouldUseSafariStylisticSetFallback(obj, props) {
    const hasVariableAxes =
      props.variableAxes && Object.keys(props.variableAxes).length > 0;
    if (hasVariableAxes || !this.isSafariBrowser()) {
      return false;
    }

    if (!this.hasEnabledStylisticSet(props.openTypeFeatures)) {
      return false;
    }

    const fontInfo = this.fonts.get(obj.fontFamily);
    return !!(fontInfo && fontInfo.font && typeof fontInfo.font.getPath === "function");
  }

  drawTextWithOpenTypePath(obj, props) {
    const fontInfo = this.fonts.get(obj.fontFamily);
    if (!fontInfo || !fontInfo.font) return false;

    try {
      const font = fontInfo.font;
      const features = this.getEnabledFeatureFlags(props.openTypeFeatures);
      const lineHeightPx = this.getLineHeightPixels(props);
      const lines = this.getTextLines(obj.text);
      const ascent = (font.ascender / font.unitsPerEm) * props.fontSize;
      ...
      lines.forEach((line, index) => {
        const lineY = props.y + index * lineHeightPx;
        const advanceWidth =
          typeof font.getAdvanceWidth === "function"
            ? font.getAdvanceWidth(line, props.fontSize, { features })
            : this.ctx.measureText(line).width;

        let lineX = props.x;
        if ((obj.textAlign || "left") === "center") {
          lineX -= advanceWidth / 2;
        } else if ((obj.textAlign || "left") === "right") {
          lineX -= advanceWidth;
        }

        const baselineY = lineY + ascent;
        const path = font.getPath(line, lineX, baselineY, props.fontSize, {
          features,
        });
        path.draw(this.ctx);
      });
      ...
      console.warn("Safari stylistic set fallback failed:", error);
      return false;
    }
  }

  applyFontFeaturesToTempCanvas(
    canvas,
    fontFamily,
    fontSize,
    variableAxes = {},
  ...
  getTextBounds(props, metrics, textAlign = "left") {
    ...
    return {
      ...
      top: props.y - padding,
      right: right,
      bottom: props.y + metrics.height + padding,
    };
  }
  ...
  updateObjectProperty(obj, property, value) {
    ...
    const hasKeyframes =
      obj.keyframes[property] && obj.keyframes[property].length > 0;
    ...
    if (!hasKeyframes) {
      ...
      const wasNewKeyframe = this.setKeyframe(
        obj,
        property,
        this.currentFrame,
        value,
  ...
  redraw() {
    ...
    if (this._originalCanvasStyles) {
      ...
      canvas.style.fontVariationSettings =
        this._originalCanvasStyles.fontVariationSettings || "";
      canvas.style.fontFeatureSettings =
        this._originalCanvasStyles.fontFeatureSettings || "";
  ...
  drawTextObject(obj) {
    ...
    const fontAvailable =
      this.fonts.has(obj.fontFamily) && this.isFontLoaded(obj.fontFamily);
    ...
    if (fontAvailable) {
      ...
      if (hasVariableAxes || hasOpenTypeFeatures) {
        this.applyFontFeaturesToCanvas(
          obj.fontFamily,
          props.fontSize,
          props.variableAxes,
          props.openTypeFeatures,
    ...
    if (this.shouldUseSafariStylisticSetFallback(obj, props)) {
      const rendered = this.drawTextWithOpenTypePath(obj, props);
      if (rendered) {
        this.ctx.restore();
        return;
      }
    ...
    if (this._pendingOpenTypeFeatures) {
      this.renderTextWithOpenTypeFeatures(
        obj.text,
        props.x,
        props.y,
        lineHeightPx,
      ...
      this.drawMultilineText(obj.text, props.x, props.y, lineHeightPx);
    }
  ...
  renderTextWithOpenTypeFeatures(text, x, y, lineHeightPx, featureSettings) {
    try {
      ...
      // Render text
      this.drawMultilineText(text, x, y, lineHeightPx);
  ...
  applyFontFeaturesToCanvas(
    ...
    try {
      const hasVariableAxes =
        variableAxes && Object.keys(variableAxes).length > 0;
      const hasOpenTypeFeatures =
        openTypeFeatures && Object.keys(openTypeFeatures).length > 0;
      ...
      if (hasOpenTypeFeatures) {
        this.applyOpenTypeFeaturesToCanvas(
          fontFamily,
          fontSize,
          openTypeFeatures,
        );
  ...
  getObjectPropertiesAtFrame(obj, frame) {
    const props = {
      ...
      lineHeight: this.getPropertyValue(obj, "lineHeight", frame),
  ...
  interpolateProperties(start, end, t, curve = null) {
    ...
    return {
      ...
      lineHeight: start.lineHeight + (end.lineHeight - start.lineHeight) * t,
      color: this.interpolateColor(start.color, end.color, t),
      variableAxes: this.interpolateVariableAxes(
        start.variableAxes || {},
        end.variableAxes || {},
        t,
      ),
  ...
  restoreSelection(previousSelection) {
    ...
    if (previousSelection && previousSelection.id) {
      const matchingObject = this.textObjects.find(
        (obj) => obj.id === previousSelection.id,
      );
      if (matchingObject) {
  ...
  setupTimelineDivider() {
    ...
    const doDrag = (e) => {
      ...
      const newHeight = Math.max(
        100,
        Math.min(600, startTimelineHeight + deltaY),
      );
    ...
    if (window.AppSettings) {
      ...
      if (savedHeight && savedHeight !== 220) {
  ...
  newProject() {
    ...
    this.canvasBackground =
      window.AppSettings?.get("canvasBackground") || "#ffffff";
    this.frameRate = 30;
    this.duration = 5;
    this.history = [];
    this.historyIndex = -1;
  ...
  loadProject(event) {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (window.UIManager) {
          window.UIManager.createNotification(
            "Error loading project file: " + error.message,
            "error",
          );
        }
      }
  ...
  loadProjectData(project) {
    const missingFonts = project.fonts.filter(
      (fontName) => !this.fonts.has(fontName),
    );

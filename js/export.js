...
class ExportManager {
  ...
  async export() {
    ...
    try {
      ...
      if (!this.app.textObjects || this.app.textObjects.length === 0) {
        throw new Error(
          "No text objects to export. Please add some text first.",
        );
      }

      if (!this.app.totalFrames || this.app.totalFrames <= 0) {
        throw new Error(
          "Invalid animation duration. Please check your timeline settings.",
        );
      }
      ...
      if (this.supportsVideoRecording()) {
        ...
        if (window.UIManager) {
          window.UIManager.createNotification(
            "Video recording not supported. Exporting as image sequence...",
            "info",
          );
        }
      ...
      if (errorMessage.includes("MediaRecorder")) {
        ...
        errorMessage +=
          " There may be an issue with canvas recording in your browser.";
      } else if (errorMessage.includes("timeout")) {
        errorMessage +=
          " The export is taking too long. Try reducing the animation duration or quality.";
      }
      ...
      if (window.UIManager) {
        ...
        if (
          ...
          !error.message?.includes("frame sequence")
        ) {
          setTimeout(() => {
            if (
              confirm(
                "Video export failed. Would you like to try exporting as image frames instead?",
              )
            ) {
              this.isExporting = false;
              this.exportWithFrameSequence().catch((seqError) => {
                ...
                window.UIManager.createNotification(
                  "Frame sequence export also failed: " + seqError.message,
                  "error",
                );
              });
            }
  ...
  async showFormatSelectionDialog() {
    return new Promise((resolve) => {
      ...
      supportedFormatKeys.forEach((format, index) => {
        ...
        const shouldSelect =
          (this.lastSelectedFormat && format === this.lastSelectedFormat) ||
  ...
  async exportWithVideoRecording(selectedFormat = null) {
    ...
    if (canvasArea > maxSupportedArea) {
      throw new Error(
        `Canvas size (${this.app.canvasWidth}×${this.app.canvasHeight}) may be too large for video export. Try reducing to 4K (3840×2160) or smaller.`,
      );
    }

    if (canvasArea > maxRecommendedArea && selectedFormat?.includes("mp4")) {
      console.warn(
        "Large canvas size detected. MP4 export may be unstable. Consider using WebM format instead.",
      );
    }
    ...
    try {
      ...
      const result = await this.exportFrameByFrame(
        canvas,
        context,
        selectedFormat,
      );
  ...
  measureMultilineText(context, text, props) {
  ...
  drawMultilineText(context, text, x, y, lineHeightPx) {
    const lines = this.getTextLines(text);
  ...
  shouldUseSafariStylisticSetFallback(textObject, props) {
  ...
  drawTextWithOpenTypePath(context, textObject, props) {
    const fontInfo = this.app.fonts.get(textObject.fontFamily);
    ...
    try {
      ...
      const lineHeightPx = this.getLineHeightPixels(props);
      const lines = this.getTextLines(textObject.text);
      ...
      lines.forEach((line, index) => {
        ...
        let lineX = props.x;
        if ((textObject.textAlign || "left") === "center") {
          lineX -= advanceWidth / 2;
        } else if ((textObject.textAlign || "left") === "right") {
          lineX -= advanceWidth;
        }
      ...
      console.warn("Safari stylistic set export fallback failed:", error);
      return false;
    }
  }
  ...
  renderTextObjectAtFrame(textObject, frame, context) {
    ...
    console.log(
      "Export: Rendering text object at frame",
      frame,
      "with props:",
      {
        x: props.x,
    ...
    if (props.rotation && props.rotation !== 0) {
      ...
      const metrics = this.measureMultilineText(
        context,
        textObject.text,
        props,
      );
      const centerX = props.x + metrics.width / 2;
      const centerY = props.y + metrics.height / 2;
    ...
    if (this.app.fonts.has(textObject.fontFamily)) {
      console.log(
        "Export: Font available, applying font features for",
      ...
      console.log(
        "Export: Font not available, using fallback for",
        textObject.fontFamily,
      );
    ...
    if (this.shouldUseSafariStylisticSetFallback(textObject, props)) {
      const rendered = this.drawTextWithOpenTypePath(context, textObject, props);
      if (rendered) {
  ...
  cleanupCanvasStyles(context) {
    ...
    if (this._originalExportCanvasStyles) {
      canvas.style.fontFamily =
        this._originalExportCanvasStyles.fontFamily || "";
      canvas.style.fontVariationSettings =
        this._originalExportCanvasStyles.fontVariationSettings || "";
      canvas.style.fontFeatureSettings =
        this._originalExportCanvasStyles.fontFeatureSettings || "";
    ...
    try {
      ...
      if (this._exportFontCache && !this._exportFontCache.has(cacheKey)) {
        try {
          ...
          console.warn(
            "Export: FontFace API not supported or failed:",
            fontFaceError,
          );
  ...
  async renderAnimationToCanvas(canvas, context, onComplete) {
    ...
    if (this.isChrome && is4K) {
      console.log(
        "Chrome detected with 4K canvas - using browser-synced rendering to prevent frame drops",
      );
    }
    ...
    const renderNextFrame = (currentTime) => {
      if (frame >= this.app.totalFrames) {
        ...
        console.log(
          `Actual duration: ${actualDuration.toFixed(2)}s, Expected: ${expectedDuration.toFixed(2)}s`,
        );
        onComplete();
        return;
      }
  ...
  async createZipSequence(frames) {
    ...
    if (window.UIManager) {
      window.UIManager.createNotification(
        `Exported ${frames.length} frames as ZIP`,
        "success",
      );
    }
  }
  ...
  async exportIndividualFrames(frames) {
    ...
    if (window.UIManager) {
      window.UIManager.createNotification(
        `Exported ${samplesToExport} sample frames`,
        "info",
      );
    }
  }
  ...
  async exportFrameByFrame(canvas, context, selectedFormat) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const baseBitrate = selectedFormat.includes("mp4")
          ? 12000000
          : 10000000;
        ...
        console.log(
          `MediaRecorder settings: ${canvas.width}x${canvas.height}, ${(bitrate / 1000000).toFixed(1)}Mbps, ${this.app.frameRate}fps`,
        );
        ...
        mediaRecorder.onstop = () => {
          console.log(
            `Recording stopped. Total chunks: ${recordedChunks.length}`,
          );

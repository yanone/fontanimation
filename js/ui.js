...
class UIManager {
  ...
  static updateTextProperties(textObject, app) {
    ...
    const currentLineHeight = app.getPropertyValue(textObject, "lineHeight");
  ...
  static updateVariableAxes(textObject, app) {
    ...
    Object.entries(axes).forEach(([tag, axisInfo]) => {
      const axisControl = UIManager.createAxisControl(
        tag,
        axisInfo,
        textObject,
        app,
  ...
  static createAxisControl(tag, axisInfo, textObject, app) {
    ...
    rangeInput.value =
      app.getPropertyValue(textObject, propertyName) || axisInfo.default;
    ...
    const updateValueVisual = (value) => {
      ...
      UIManager.updateAxisValueVisual(
        propertyName,
        parseFloat(value),
        textObject,
        app,
    ...
    const handleNumberInputChange = (e) => {
      const value = Math.max(
        axisInfo.min,
        Math.min(axisInfo.max, parseFloat(e.target.value) || axisInfo.default),
      );
  ...
  static updateOpenTypeFeatures(textObject, app) {
    ...
    const fontInfo = app.fonts.get(textObject.fontFamily);
    const features = {
      ...fontInfo.openTypeFeatures,
      ...fontInfo.stylisticSets,
    };
    ...
    sortedFeatures.forEach(([tag, featureInfo]) => {
      const featureControl = UIManager.createFeatureControl(
        tag,
        featureInfo,
        textObject,
        app,
  ...
  static createFeatureControl(tag, featureInfo, textObject, app) {
    ...
    checkbox.checked =
      (textObject.openTypeFeatures && textObject.openTypeFeatures[tag]) ||
      false;
  ...
  static updateKeyframeButtonStates(textObject, app) {
    ...
    document.querySelectorAll(".keyframe-btn").forEach((btn) => {
      ...
      if (property) {
        const hasKeyframe = app.hasKeyframe(
          textObject,
          property,
          app.currentFrame,
        );
  ...
  static updateTransitionButtonStates(textObject, app) {
    ...
    document.querySelectorAll(".transition-btn").forEach((btn) => {
      ...
      if (property) {
        ...
        const isLastKeyframe = UIManager.isLastKeyframe(
          textObject,
          property,
          app.currentFrame,
        );
        const hasCustomCurve = UIManager.hasCustomTransition(
          textObject,
          property,
          app.currentFrame,
        );
  ...
  static hasCustomTransition(textObject, property, frame) {
    ...
    const keyframe = textObject.keyframes[property].find(
      (kf) => kf.frame === frame,
    );
    return (
      keyframe &&
  ...
  static isLastKeyframe(textObject, property, frame) {
    if (
      ...
      textObject.keyframes[property].length === 0
    ) {
      return false;
    }
    ...
    const lastKeyframe = keyframes.reduce((max, kf) =>
      kf.frame > max.frame ? kf : max,
    );
